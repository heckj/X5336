...
class Parser {
  ...
  private func declaration() throws -> Statement? {
    ...
    do {
      ...
      synchronize()
      errors.append(
        ErrorInfo(error: error, position: errorPosition, syncPosition: current, count: tokens.count)
      )
      return nil
    }
  }
  ...
  private func statement() throws -> Statement {
    ...
    if omgVerbose {
      indent()
      print("fork -> expressionStatement()")
    }
  ...
  private func forStatement() throws -> Statement {
    ...
    if match(.SEMICOLON) {
      ...
      if omgVerbose {
        indent()
        print("fork -> variableDeclaration()")
      }
    ...
    if !check(.SEMICOLON) {
      ...
      if omgVerbose {
        indent()
        print("Expression.literal for condition")
      }
  ...
  private func returnStatement() throws -> Statement {
    ...
    if omgVerbose {
      indent()
      print("Statement.returnStatement")
      omgIndent -= 1
    }
  ...
  private func expressionStatement() throws -> Statement {
    ...
    if omgVerbose {
      indent()
      print("Statement.expressionStatement")
      omgIndent -= 1
    }
  ...
  private func assignment() throws -> Expression {
    ...
    if match(.EQUAL) {
      ...
      default:
        ...
        errors.append(
          ErrorInfo(
            error: ParserError.invalidAssignmentToken(equals),
            position: current,
  ...
  private func primary() throws -> Expression {
    ...
    default:
      if omgVerbose {
        indent()
        print("NO MORE PRIMARY TOKENS - Error")
      }
      throw ParserError.unexpectedToken(
        current, [.FALSE, .TRUE, .NIL, .STRING, .NUMBER, .IDENTIFIER, .LEFT_PAREN],
        message: "Unable to match primary() token")
    }
  }
